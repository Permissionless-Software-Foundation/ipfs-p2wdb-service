  ...
  describe('POST /users - Create User', () => {
    ...
    it('should reject signup if no password property is provided', async () => {
      try {
        ...
        assert.equal(err.response.status, 422)
        assert.include(err.response.data, "Property 'password' must be a string")
      }
    })
    ...
    it("should signup of type 'user' by default", async () => {
      ...
      assert(result.data.user.email === 'test3@test.com', 'Email of test expected')
      assert(result.data.user.password === undefined, 'Password expected to be omited')
  ...
  describe('PUT /users/:id', () => {
    ...
    it('should not be able to update user type', async () => {
      try {
        ...
        assert.equal(err.response.status, 422)
        assert.include(err.response.data, "Property 'type' can only be changed by Admin user")
      }
    })
