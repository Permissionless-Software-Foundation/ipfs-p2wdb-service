...
describe('#validators', () => {
  ...
  describe('#constructor', () => {
    it('should throw an error if adapters is not passed in.', () => {
      try {
        ...
        assert.include(err.message, 'Instance of Adapters library required when instantiating JSON RPC Validators library.')
      }
    })
  })
  describe('#ensureUser', () => {
    ...
    it('should throw an error if JWT token can not be decoded', async () => {
      try {
        const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjYwNmQxYTlkNTgxNTVjMjIzNWFmMTNhMSIsImlhdCI6MTYxNzc2Mjk3M30.6JkM1v0n71Mzsd3qzClzlMKtq6HlD0umoauG23N9FFF'
        // Generate the parsed data that the main router would pass to this
        // endpoint.
        const id = uid()
        const userCall = jsonrpc.request(id, 'users', {
  ...
  describe('#ensureTargetUserOrAdmin', () => {
    ...
    it('should throw error if JWT token can not be decoded', async () => {
      try {
        const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjYwNmU0YzkxYzdlYWNjN2Q4NWJjOGI0NCIsImlhdCI6MTYxNzg0MTI5N30.n1sas7YlqtmhBlNDBY_IXxQCrIQTiE8UITqy0PJAFFF'
        // Generate the parsed data that the main router would pass to this
        // endpoint.
        const id = uid()
        const userCall = jsonrpc.request(id, 'users', {
