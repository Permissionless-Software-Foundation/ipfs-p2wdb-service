/*
  This file copied from orbitdb/src/oplog/log.js,
 ...
 */
import LRU from 'lru'
import Entry from '@chris.troutner/orbitdb-helia/src/oplog/entry.js'
import Clock, { tickClock } from '@chris.troutner/orbitdb-helia/src/oplog/clock.js'
import Heads from '@chris.troutner/orbitdb-helia/src/oplog/heads.js'
import ConflictResolution from '@chris.troutner/orbitdb-helia/src/oplog/conflict-resolution.js'
import MemoryStorage from '@chris.troutner/orbitdb-helia/src/storage/memory.js'
...
const Log = async (identity, { logId, logHeads, access, entryStorage, headsStorage, indexStorage, sortFn } = {}) => {
  ...
  const get = async (hash) => {
    ...
    // bytes here is the binary encoded version of the entry.
    const bytes = await _entries.get(hash)
    // console.log('bytes: ', bytes)
    if (bytes) {
      // Decode the entry binary bytes into an object.
      ...
      if (localP2wdbCanAppend) {
        // Validate the entry.
        const isValid = await localP2wdbCanAppend.canAppend(entry)
  ...
  const append = async (data, options = { referencesCount: 0 }) => {
    ...
    console.log('append() calling _entries.put()')
  ...
  const join = async (log) => {
    ...
    if (_entries.merge) {
      console.log('join() calling _entries.merge()')
  ...
  const joinEntry = async (entry) => {
    ...
    console.log('joinEntry() Calling _entries.put()')
  ...
  const injectDeps = (p2wdbCanAppend) => {
    console.log('op-log injectDeps() executed.')
