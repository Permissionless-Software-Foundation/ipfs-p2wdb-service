...
class JSONRPC {
  constructor (localConfig = {}) {
    ...
    if (!this.adapters) {
      throw new Error('Instance of Adapters library required when instantiating JSON RPC Controllers.')
    }
    this.useCases = localConfig.useCases
    if (!this.useCases) {
      throw new Error('Instance of Use Cases library required when instantiating JSON RPC Controllers.')
    }
    ...
    this.entryController = new EntryRPC(localConfig)
  ...
  async router (str, from) {
    try {
      ...
      if (!from || typeof from !== 'string') {
        wlogger.info('Warning: Can not send JSON RPC response. Can not determine which peer this message came from.')
        return false
      }
      ...
      if (alreadyProcessed) {
        ...
        if (parsedData.type.includes('success') &&
                    parsedData.payload.method === undefined) {
          return false
        }
        // Log the incoming JSON RPC command.
        wlogger.info(`JSON RPC received from ${from}, ID: ${parsedData.payload.id}, type: ${parsedData.type}, method: ${parsedData.payload.method}`)
      }
      ...
      switch (parsedData.payload.method) {
        ...
        case 'about':
          ...
          break
        case 'p2wdb':
          retObj = await _this.entryController.entryRouter(parsedData)
          break
      }
      ...
      try {
        await _this.ipfsCoord.useCases.peer.sendPrivateMessage(from, retStr, thisNode)
      } catch (err) {
